              ...
              <div className='w-full flex flex-col md:flex-row gap-5 2xl:gap-8 bg-white shadow-md p-8 overflow-y-auto'>
                ...
                <div className='w-full md:w-1/2 space-y-6'>
                  <div className='flex items-center gap-5'>
                    <div className={clsx("flex gap-1 items-center text-base font-semibold px-3 py-1 rounded-full", PRIORITYSTYELS[task?.priority], bgColor[task?.priority])}>
                    ...
                    <div className='space-y-3'>
                      {
                        task?.team?.map((member, index) => (
                          <div key={index} className='flex gap-4 py-2 items-center border-gray-200'>
                          ...
                          <div key={index} className='flex gap-3'>
                            ...
                            <div className='space-y-1'>
                              <div className='flex gap-2 items-center'>
                                <span className='text-sm text-gray-500'>{new Date(subtask?.date).toDateString()}</span>
                ...
                <div className='w-full md:w-1/2 space-y-8'>
                  <p className='text-lg font-semibold bg-gray-700 rounded-full text-white w-fit px-3 py-2'>ASSETS</p>
                  <div className='w-full grid grid-cols-2 gap-4'>
                    {
                      task?.assets?.map((task, index) => (
                        <img key={index} src={task} alt={task?.title} className='w-full rounded h-28 md:h-36 2xl:h-52 cursor-pointer transition-all duration-700 hover:scale-125 hover:z-50' />
                      ))
                    }
                  </div>
                </div>
    ...
    return (
      <div className="flex space-x-4">
        <div className="flex flex-col items-center flex-shrink-0">
          ...
          {isConnected && (
            <PiLineVerticalThin className="w-0.5 font-light bg-purple-500 h-full" />
          )}
        </div>
  ...
  return (
    <div className='w-full flex gap-10 2xl:gap-20 min-h-screen px-10 py-8 bg-white shadow-md rounded-md justify-between overflow-y-auto'>
      <div className="w-full md:w-1/2">
        <h4 className="bg-gray-800 w-fit rounded-lg text-white font-semibold text-lg mb-5 px-2 py-2">
          Activities
        </h4>
        <div className="w-full">
          {activity?.map((elem, index) => (
            <Card key={index} item={elem} isConnected={index < activity.length - 1} />
          ))}
        </div>
      </div>
