...
const NotificationPanel = () => {
    const readNotificationHandler = (type, id) => {
        //TODO: Mark notification as read 
    }
    const viewNotificationHandler = (item) => {
        //TODO: View notification logic
    }
    const callsToAction = [
    ...
    return (
        <Popover className="realtive">
            <PopoverButton className="inline-flex items-center outline-none">
                <div className='w-8 h-8 flex items-center justify-center text-gray-800 relative md:mr-2'>
                                    ...
                                    <div className='grid grid-cols-2 divide-x bg-gray-50'>
                                        {callsToAction.map((item) => (
                                            <Link
                                                key={item.name}
                                                onClick={
                                                    item?.onClick ? () => item.onClick() : () => props.close()
                                                }
